...
import { AlertTriangle, Ticket, Users, Clock, MapPin, X, ChevronRight, Activity, LogIn, ShieldCheck, Hand, Swords, MoreHorizontal, FileDown, Bell, BellOff } from "lucide-react";
...
groups[key].push({ ...s, words });
}
const clusters = [];
Object.entries(groups).forEach(([station, items]) => {
if (items.length < 2) return;
for (let i = 0; i < items.length; i++) {
for (let j = i + 1; j < items.length; j++) {
const shared = [...items[i].words].filter((w) => items[j].words.has(w));
if (shared.length >= 2) clusters.push({ station, a: items[i], b: items[j], shared });
}
}
});
return clusters;
...
} catch { /_ silence si le contexte audio n'est pas disponible _/ }
}
...
} catch { /_ notif indisponible, le son suffit _/ }
}
}
...
const left = 15;
      ...
      autoTable(doc, {
        startY: y + 4,
        head: [["Station", "Lignes", "Mots communs", "Description A", "Date/heure A", "Description B", "Date/heure B"]],
        body: clusters.length > 0
          ? clusters.map((c) => [
              c.station,
              `L${c.a.ligne} / L${c.b.ligne}`,
              c.shared.join(", "),
              c.a.description.slice(0, 40),
              fmtDateTime(new Date(c.a.created_at).getTime()),
              c.b.description.slice(0, 40),
              fmtDateTime(new Date(c.b.created_at).getTime()),
            ])
...
return (
<div style={{ fontFamily: "'Inter', sans-serif", background: "#0A0D10", minHeight: "100vh", color: "#E8ECEF" }}>
<style>{FONT_IMPORT}{`        @keyframes pulse-ring { 0% { box-shadow: 0 0 0 0 rgba(255,90,46,0.55);} 70% { box-shadow: 0 0 0 10px rgba(255,90,46,0);} 100% { box-shadow: 0 0 0 0 rgba(255,90,46,0);} }
     ...
     `}</style>
      ...
      <div style={{ borderBottom: "1px solid #1E262D", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#0A0D10", zIndex: 20, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span className="mono" style={{ fontSize: 12, color: "#7C8892" }}>{profile ? `${profile.matricule} · ${profile.nom}` : "…"}</span>
      ...
      {tab === "admin" && profile?.role === "admin" ? (
        ...
        <>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 20, padding: 20 }}>
            ...
            <div style={{ flex: "1 1 560px", background: "#12171C", border: "1px solid #1E262D", borderRadius: 10, padding: "28px 24px 24px" }}>
              <div className="disp" style={{ fontSize: 14, color: "#5B6670", textTransform: "uppercase", marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ flex: "1 1 320px", display: "flex", flexDirection: "column", gap: 10, maxHeight: 480, overflowY: "auto" }}>
                  ...
                  <div key={s.id} style={{ background: "#12171C", border: "1px solid #1E262D", borderLeft: `4px solid ${statutInfo.color}`, borderRadius: 8, padding: 12 }}>
                    ...
                    {s.statut !== "nouveau" && s.statut_agent && (
                      <div style={{ fontSize: 11, color: "#5B6670", marginTop: 6 }}>
                        {statutInfo.action} <span style={{ color: "#B4BCC4" }}>{s.statut_agent.equipe} ({s.statut_agent.nom})</span>
        ...
        <div style={{ padding: 20, display: "flex", flexWrap: "wrap", gap: 20 }}>
          ...
          <div style={{ flex: "1 1 100%", background: "#12171C", border: "1px solid #1E262D", borderRadius: 10, padding: 20 }}>
            ...
            <div style={{ fontSize: 12, color: "#5B6670", marginBottom: 16 }}>Heuristique de démo : même station (toutes lignes confondues) + descriptions partageant au moins 2 mots-clés.</div>
            {clusters.length === 0 && <div style={{ fontSize: 13, color: "#5B6670" }}>Aucune récurrence détectée sur cette période.</div>}
            {clusters.map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #1E262D" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#FFC145", flexShrink: 0 }} />
                <div style={{ fontSize: 13 }}>
                  <span style={{ fontWeight: 600 }}>{c.station}</span>
                  <span style={{ color: "#7C8892" }}> — possible même groupe : </span>
                  <span className="mono" style={{ fontSize: 10, color: "#7C8892" }}>[L{c.a.ligne}]</span>
                  <span style={{ color: "#7C8892" }}> "{c.a.description.slice(0, 40)}…" </span>
                  <ChevronRight size={12} style={{ verticalAlign: -1 }} />
                  <span className="mono" style={{ fontSize: 10, color: "#7C8892" }}> [L{c.b.ligne}]</span>
                  <span style={{ color: "#7C8892" }}> "{c.b.description.slice(0, 40)}…"</span>
                  <div className="mono" style={{ fontSize: 11, color: "#5B6670", marginTop: 2 }}>
                    mots communs : {c.shared.join(", ")} · {fmtDateTime(new Date(c.a.created_at).getTime())} → {fmtDateTime(new Date(c.b.created_at).getTime())}
                  </div>
                </div>
              </div>
            ))}
