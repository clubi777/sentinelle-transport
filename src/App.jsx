...
const TYPES = {
  ...
  vente: {
    label: 'Vente illégale de titres',
    short: 'VTT',
    icon: Ticket,
    color: '#FFC145',
  },
...
const STATUTS = {
  ...
  pris_en_charge: {
    ...
    color: '#FFC145',
    icon: Clock,
    action: 'Pris en charge par',
  },
  interpellation: {
    label: 'Interpellation',
    color: '#23C9A7',
    icon: ShieldCheck,
    action: 'Interpellé par',
  },
  faux_positif: {
    label: 'Faux positif',
    color: '#5B6670',
    icon: X,
    action: 'Classé faux positif par',
  },
};
...
function computeRecurrence(signalements) {
  ...
  Object.entries(byStation).forEach(([station, items]) => {
    ...
    Object.values(byRoot).forEach((indices) => {
      ...
      indices.forEach((i) =>
        indices.forEach((j) => {
          if (i < j && edgeWords[`${i}-${j}`])
            edgeWords[`${i}-${j}`].forEach((w) => sharedWords.add(w));
        })
      );
      result.push({
        station,
...
export default function Sentinelle() {
  ...
  const [notifEnabled, setNotifEnabled] = useState(
    () => localStorage.getItem('sentinelle_notif') !== 'off'
  );
  const [notifScope, setNotifScope] = useState(
    () => localStorage.getItem('sentinelle_notif_scope') || 'active'
  ); // "active" | "all"
  ...
  useEffect(() => {
    notifScopeRef.current = notifScope;
  }, [notifScope]);
  useEffect(() => {
  ...
  function playBeep() {
    try {
      ...
      /* silence si le contexte audio n'est pas disponible */
    }
  }
  ...
  function notifyNewSignalement(row) {
    ...
    if (
      ...
      try {
        ...
        /* notif indisponible, le son suffit */
      }
    }
  }
  ...
  const fetchSignalements = useCallback(async () => {
    const { data, error } = await supabase
      .from('signalements')
      .select(
        '*, agents!signalements_agent_id_fkey(nom, equipe), statut_agent:agents!signalements_statut_agent_id_fkey(nom, equipe)'
      )
  ...
  const lineSignalements = useMemo(
    () => last24h.filter((s) => s.ligne === activeLine),
    [last24h, activeLine]
  );
  ...
  const clusters = useMemo(
    () => computeRecurrence(searchResults),
    [searchResults]
  );
  ...
  const canEditStatut =
    profile?.role !== 'agent_station' && profile?.role !== 'operateur_video';
  ...
  async function exportStatsPDF() {
    ...
    try {
      ...
      if (logo) {
        try {
          doc.addImage(logo, 'PNG', left, 10, 16, 16);
        } catch {
      ...
      const periodLabel =
        searchStart === searchEnd
          ...
          : `Période : du ${new Date(searchStart).toLocaleDateString(
              'fr-FR'
            )} au ${new Date(searchEnd).toLocaleDateString('fr-FR')}`;
      const filtersLabel = [
      ...
      doc.setTextColor(...navy);
      doc.text(
        periodLabel + (filtersLabel ? ` — ${filtersLabel}` : ''),
        left,
        37
      );
      doc.setTextColor(...gray);
      ...
      doc.setTextColor(...navy);
      doc.text(
        `${searchResults.length} signalement${
      ...
      const breakdownLine =
        Object.entries(TYPES)
          .filter(([k]) => searchBreakdown[k] > 0)
          .map(([k, v]) => `${searchBreakdown[k]} ${v.label.toLowerCase()}`)
          .join(' · ') || 'aucun détail';
      ...
      autoTable(doc, {
        ...
        body: Object.entries(statsStationCounts)
          .sort((a, b) => b[1] - a[1])
          .map(([key, count]) => {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setTextColor(...gray);
        doc.text(
          'Sentinelle Transport — document généré automatiquement, usage interne',
          left,
          290
        );
        doc.text(`Page ${i}/${pageCount}`, pageWidth - left, 290, {
          align: 'right',
        });
      }

      doc.save(
        `sentinelle-transport-rapport-${new Date()
          .toISOString()
          .slice(0, 10)}.pdf`
      );
    } finally {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Sentinelle Transport
            </span>
          </div>

          <div style={{ fontSize: 12, color: '#5B6670', marginBottom: 18 }}>
            Connexion avec le matricule et le mot de passe fournis par un
            administrateur.
          </div>

          <label style={{ fontSize: 12, color: '#7C8892' }}>Matricule</label>
          <input
            value={authForm.matricule}
            onChange={(e) =>
              setAuthForm({ ...authForm, matricule: e.target.value })
            }
            placeholder="Ex. GP4521"
            style={{
              width: '100%',
              background: '#0A0D10',
              color: '#E8ECEF',
          ...
          {authError && (
            <div style={{ fontSize: 12, color: '#FF5A2E', marginBottom: 12 }}>
              {authError}
            </div>
          )}
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {dataError && (
            <span style={{ fontSize: 11, color: '#FF5A2E' }}>{dataError}</span>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span className="mono" style={{ fontSize: 12, color: '#7C8892' }}>
            {profile
              ? `${profile.matricule} · ${
          ...
          {pendingCount > 0 && (
            <span
              title="Signalement(s) en attente d'envoi — seront transmis automatiquement dès que le réseau revient"
              className="mono"
              style={{
                fontSize: 11,
                color: '#FFC145',
          ...
          {profile?.role !== 'agent_station' && notifEnabled && (
            <button
              onClick={cycleNotifScope}
              title="Portée des alertes sonores"
              style={{
                background: 'none',
                border: '1px solid #1E262D',
                color: '#5B6670',
          ...
          <button
            ...
            style={{
              background: 'none',
              border: '1px solid #1E262D',
              color: notifEnabled ? '#FF5A2E' : '#5B6670',
              borderRadius: 6,
              padding: '6px 10px',
              cursor: 'pointer',
              display: 'flex',
          ...
          <button
            onClick={() => {
              setForm({
                line: activeLine,
                station: LINES[activeLine].stations[0],
                type: 'pickpocket',
                nb: 1,
                desc: '',
              });
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              borderBottom:
                tab === key ? '2px solid #FF5A2E' : '2px solid transparent',
            }}
          >
            {label}
          </button>
      ...
      {tab === 'admin' && profile?.role === 'admin' ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', color: '#5B6670' }}>
        ...
        <>
            ...
            >
              <span
                style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                  background: LINES[activeLine].color,
                  color: LINES[activeLine].dark ? '#fff' : '#0A0D10',
                  fontWeight: 700,
                  fontSize: 12,
                }}
                className="mono"
              ...
              <span style={{ fontSize: 12, color: '#5B6670' }}>
                Ligne affectée — accès limité à {LINES[activeLine].name}
              </span>
            </div>
          ) : (
            <div
          ...
          <div
            style={{ display: 'flex', flexWrap: 'wrap', gap: 20, padding: 20 }}
          >
            ...
            >
              ...
              >
                <MapPin size={14} /> {line.name} — {line.stations[0]} ↔{' '}
              ...
              <div>
                {line.stations.map((name, i) => {
                  const count =
                    liveStationCounts[`${activeLine}__${name}`] || 0;
                  const isLast = i === line.stations.length - 1;
                  const pins = lineSignalements.filter(
                    (s) => s.station === name && s.statut !== 'faux_positif'
                  );
                  return (
                      ...
                      <div style={{ flex: 1, paddingBottom: 18 }}>
                        ...
                        >
                          ...
                          {count > 0 && (
                            <span
                              className="mono"
                              style={{ fontSize: 10, color: '#5B6670' }}
                            >
                              {count} sign.
                            </span>
                          )}
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: '#5B6670', marginTop: 2 }}>
                  Signalements des dernières 24 heures
                </div>
              </div>
              {last24h.map((s) => {
                const tsMs = new Date(s.created_at).getTime();
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span
                          ...
                          style={{
                            fontSize: 10,
                            background: LINES[s.ligne].color,
                            color: LINES[s.ligne].dark ? '#fff' : '#0A0D10',
                            borderRadius: 4,
                            padding: '1px 5px',
                          }}
                        >
                    ...
                    >
                      <span
                        className="mono"
                        style={{ fontSize: 11, color: '#5B6670' }}
                      >
                        <Users
                          size={11}
                          style={{ verticalAlign: -2, marginRight: 3 }}
                        />
                        {s.nb_personnes} · {equipeLabel(s)} · {fmtTime(tsMs)}
                      </span>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <button
                          onClick={() => duplicateSignalement(s)}
                          title="Dupliquer ce signalement — pré-remplit un nouveau signalement avec les mêmes infos"
                          style={{
                            background: 'none',
                            border: '1px solid #1E262D',
                            color: '#7C8892',
                    ...
                    {s.statut !== 'nouveau' && s.statut_agent && (
                      <div
                        style={{ fontSize: 11, color: '#5B6670', marginTop: 6 }}
                      >
                        {statutInfo.action}{' '}
                        <span style={{ color: '#B4BCC4' }}>
                          {canEditStatut
                            ? `${s.statut_agent.equipe} (${s.statut_agent.nom})`
                            : s.statut_agent.equipe}
                        </span>
                        {s.statut_updated_at && (
                          <>
                            {' '}
                            · {fmtTime(new Date(s.statut_updated_at).getTime())}
                          </>
                        )}
                      </div>
                    )}
              ...
              {last24h.length === 0 && (
                <div style={{ fontSize: 13, color: '#5B6670' }}>
                  Aucun signalement au cours des dernières 24h.
                </div>
              )}
            </div>
          </div>
        ...
        <div
          style={{ padding: 20, display: 'flex', flexWrap: 'wrap', gap: 20 }}
        >
          <div
            style={{
              flex: '1 1 100%',
          ...
          >
            ...
            >
              ...
              <div>
                ...
                >
                  <option
                    value="all"
                    style={{ color: '#0A0D10', background: '#FFFFFF' }}
                  >
                    Tous types
                  </option>
                  {Object.entries(TYPES).map(([k, v]) => (
            ...
            <div style={{ fontSize: 14, marginBottom: 12 }}>
              <strong>{searchResults.length}</strong> signalement
              ...
              <span style={{ color: '#5B6670' }}>
                ...
                {Object.entries(TYPES)
                  .filter(([k]) => searchBreakdown[k] > 0)
                  .map(
                    ([k, v]) => `${searchBreakdown[k]} ${v.label.toLowerCase()}`
                  )
                  .join(' · ') || 'aucun détail'}
              </span>
            </div>
              ...
              >
                {searchResults.map((s) => (
                  ...
                  >
                    ...
                    <span style={{ flex: 1 }}>
                      {s.station} — {TYPES[s.type].label}
                    </span>
                    <span
                      className="mono"
                      style={{ color: STATUTS[s.statut].color }}
                    >
                      {STATUTS[s.statut].label}
                    </span>
                    <span className="mono" style={{ color: '#5B6670' }}>
                      {new Date(s.created_at).toLocaleDateString('fr-FR')}{' '}
                      {fmtTime(new Date(s.created_at).getTime())}
                    </span>
                  </div>
                ))}
              </div>
          ...
          >
            ...
            {Object.entries(statsStationCounts)
              .sort((a, b) => b[1] - a[1])
              .slice(0, 12)
              .map(([key, count]) => {
                const [lineId, station] = key.split('__');
                    ...
                    >
                      <span>
                        <span
                          ...
                          style={{
                            fontSize: 10,
                            background: LINES[lineId].color,
                            color: LINES[lineId].dark ? '#fff' : '#0A0D10',
                            borderRadius: 4,
                            padding: '1px 5px',
                            marginRight: 6,
                          }}
                      ...
                      <span className="mono" style={{ color: '#7C8892' }}>
                        {count}
                      </span>
                    </div>
                    <div
            ...
            {Object.keys(statsStationCounts).length === 0 && (
              <div style={{ fontSize: 13, color: '#5B6670' }}>
          ...
          >
            ...
            {byHourBucket.map(([bucket, count]) => (
              ...
              >
                ...
                <span style={{ color: '#7C8892' }}>
                  {count} signalement{count > 1 ? 's' : ''}
                </span>
              </div>
            ))}
            {byHourBucket.length === 0 && (
              <div style={{ fontSize: 13, color: '#5B6670' }}>
                Pas de données sur cette période.
              </div>
            )}
          ...
          >
            ...
            >
              Carte de chaleur — station × heure
            </div>
            <div style={{ fontSize: 12, color: '#5B6670', marginBottom: 16 }}>
              Top 10 stations sur la période, intensité par tranche horaire
              (0h–23h). Vient en complément des compteurs ci-dessus, ne les
              remplace pas.
            </div>
              ...
              <>
                <div style={{ overflowX: 'auto' }}>
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: `150px repeat(24, 24px)`,
                      gap: 3,
                      alignItems: 'center',
                      minWidth: 150 + 24 * 27,
                    }}
                ...
                >
                  <span style={{ fontSize: 11, color: '#5B6670' }}>
                    Intensité :
                  </span>
                  {[
                    { count: 0, label: 'Aucun' },
          ...
          >
            ...
            <div style={{ fontSize: 12, color: '#5B6670', marginBottom: 16 }}>
              Signalements probablement liés au même groupe : même station
              (toutes lignes confondues), descriptions partageant au moins 2
              mots-clés.
            </div>
            {clusters.length === 0 && (
              <div style={{ fontSize: 13, color: '#5B6670' }}>
              ...
              >
                ...
                >
                  <div style={{ fontSize: 14, fontWeight: 700 }}>
                    ...
                    >
                      — {c.items.length} signalements liés
                    </span>
                  </div>
                  ...
                  {c.items.map((item, idx) => (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{
                          fontSize: 9,
                          background: LINES[item.ligne].color,
                          color: LINES[item.ligne].dark ? '#fff' : '#0A0D10',
                          borderRadius: 3,
                          padding: '2px 5px',
                          flexShrink: 0,
                        }}
                      ...
                      <span
                        ...
                        style={{
                          fontSize: 9,
                          fontWeight: 700,
                          background: TYPES[item.type].color,
                          color: '#0A0D10',
                          borderRadius: 3,
                          padding: '2px 5px',
                          flexShrink: 0,
                      ...
                      >
                        {TYPES[item.type].short}
                      </span>
                      <span style={{ color: '#B4BCC4', flex: 1 }}>
                        {item.description}
                      </span>
                      <span
                        className="mono"
        ...
        >
          ...
          >
            ...
            >
              ...
              <X
                size={18}
                style={{ cursor: 'pointer', color: '#5B6670' }}
            ...
            <select
              disabled={profile?.role === 'agent_station'}
              value={form.line}
              onChange={(e) =>
                setForm({
                  ...form,
                  line: e.target.value,
                  station: LINES[e.target.value].stations[0],
                })
              }
              style={{
                width: '100%',
                background:
                  profile?.role === 'agent_station' ? '#161B21' : '#0A0D10',
                color:
                  profile?.role === 'agent_station' ? '#5B6670' : '#E8ECEF',
                border: '1px solid #1E262D',
                borderRadius: 6,
                padding: 9,
                marginTop: 4,
            ...
            >
              {LINES[form.line].stations.map((s) => (
                <option
                  key={s}
                  value={s}
                  style={{ color: '#0A0D10', background: '#FFFFFF' }}
                  ...
                  style={{
                    ...
                    border:
                      form.type === k
                        ? `1px solid ${v.color}`
                        : '1px solid #1E262D',
                    background: form.type === k ? `${v.color}22` : '#0A0D10',
                    color: form.type === k ? v.color : '#7C8892',
                    fontSize: 13,
                    cursor: 'pointer',
                  }}
                >
            ...
            <label style={{ fontSize: 12, color: '#7C8892' }}>
              Nombre de personnes
            </label>
            <input
              type="number"
            ...
            <textarea
              ...
              placeholder="Vêtements, position précise, comportement observé..."
              rows={3}
              style={{
                width: '100%',
                background: '#0A0D10',
            ...
            />
            <div style={{ fontSize: 11, color: '#5B6670', marginBottom: 6 }}>
              Signalé par {profile ? `${profile.equipe} (${profile.nom})` : '…'}
            </div>
            {formError && (
              <div style={{ fontSize: 12, color: '#FF5A2E', marginBottom: 8 }}>
                {formError}
              </div>
            )}
      ...
      {selectedPin &&
        (() => {
          const pin =
            signalements.find((s) => s.id === selectedPin.id) || selectedPin;
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div
                  style={{ fontSize: 13, color: '#B4BCC4', marginBottom: 10 }}
                >
                  {pin.description}
                </div>
                <div
                  className="mono"
                  style={{ fontSize: 11, color: '#5B6670', marginBottom: 14 }}
                >
                  {LINES[pin.ligne].name} · {TYPES[pin.type].short} —{' '}
                  {TYPES[pin.type].label} · {pin.nb_personnes} pers. ·{' '}
                  {equipeLabel(pin)} ·{' '}
                  {fmtTime(new Date(pin.created_at).getTime())}
                </div>
                ...
                {canEditStatut ? (
                  <select
                    value={pin.statut}
                    onChange={(e) => updateStatut(pin.id, e.target.value)}
                    style={{
                      width: '100%',
                      background: '#0A0D10',
                      color: statutInfo.color,
                ...
                {pin.statut !== 'nouveau' && pin.statut_agent && (
                  <div style={{ fontSize: 12, color: '#7C8892' }}>
                    ...
                    <span style={{ color: '#E8ECEF' }}>
                      {pin.statut_agent.equipe} ({pin.statut_agent.nom})
                    </span>
                    {pin.statut_updated_at && (
                      <>
                        {' '}
                        · {fmtTime(new Date(pin.statut_updated_at).getTime())}
                      </>
                    )}
                  </div>
                )}
